import { NextRequest, NextResponse } from "next/server";
import axios from "axios";
import * as cheerio from "cheerio";

export async function POST(request: NextRequest) {
  try {
    const { url } = await request.json();

    if (!url) {
      return NextResponse.json({ 
        success: false, 
        error: "URLが提供されていません" 
      }, { status: 400 });
    }

    // Instagramのページをフェッチ
    const { data } = await axios.get(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
        "Accept-Language": "ja,en-US;q=0.7,en;q=0.3",
        "Accept-Encoding": "gzip, deflate, br",
        "DNT": "1",
        "Connection": "keep-alive",
        "Upgrade-Insecure-Requests": "1",
      }
    });

    const $ = cheerio.load(data);

    let mediaUrl = "";
    let thumbnailUrl = "";
    let title = "Instagram Media";
    let type: "video" | "image" | "unknown" = "unknown";

    // og:videoタグから動画URLを抽出
    $("meta[property=\"og:video\"]").each((i, el) => {
      const content = $(el).attr("content");
      if (content && content.includes("video")) {
        mediaUrl = content;
        type = "video";
        return false;
      }
    });

    // og:imageタグからサムネイルURLを抽出
    $("meta[property=\"og:image\"]").each((i, el) => {
      const content = $(el).attr("content");
      if (content) {
        thumbnailUrl = content;
        return false;
      }
    });

    // og:titleタグからタイトルを抽出
    $("meta[property=\"og:title\"]").each((i, el) => {
      const content = $(el).attr("content");
      if (content) {
        title = content;
        return false;
      }
    });

    // og:imageタグから画像URLを抽出（動画がない場合）
    if (!mediaUrl) {
      $("meta[property=\"og:image\"]").each((i, el) => {
        const content = $(el).attr("content");
        if (content && (content.includes(".jpg") || content.includes(".jpeg") || content.includes(".png"))) {
          mediaUrl = content;
          type = "image";
          return false;
        }
      });
    }

    if (mediaUrl) {
      return NextResponse.json({ 
        success: true, 
        url: mediaUrl, 
        thumbnail: thumbnailUrl, 
        title, 
        type 
      });
    } else {
      return NextResponse.json({ 
        success: false, 
        error: "動画または画像URLが見つかりませんでした。プライベート投稿やカルーセル投稿はサポートされていない場合があります。" 
      }, { status: 404 });
    }

  } catch (error: any) {
    console.error("ダウンロードエラー:", error.message);
    
    if (error.response?.status === 404) {
      return NextResponse.json({ 
        success: false, 
        error: "投稿が見つかりません。URLを確認してください。" 
      }, { status: 404 });
    }
    
    if (error.response?.status === 403) {
      return NextResponse.json({ 
        success: false, 
        error: "アクセスが拒否されました。プライベートアカウントの投稿の可能性があります。" 
      }, { status: 403 });
    }

    return NextResponse.json({ 
      success: false, 
      error: "メディアの取得中にエラーが発生しました。URLを確認してください。" 
    }, { status: 500 });
  }
}
